...
func workferAppGroupStatus(ctx context.Context, id int, chanAppGroups <-chan AppGroupStatus) {
	log.Info("START appgroup worker id : ", id)

	for chanAppGroup := range chanAppGroups {
		appGroupId := chanAppGroup.AppGroupId
		workflowId := chanAppGroup.WorkflowId
		status := chanAppGroup.Status
		statusDesc := chanAppGroup.StatusDesc
